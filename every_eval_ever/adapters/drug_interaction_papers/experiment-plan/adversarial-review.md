...
| Medium | “The module entrypoint is clean and repeatable.” | Eager imports from the package `__init__` loaded `adapter.py` before `uv run python -m ...adapter`, producing a runpy warning and weakening warning-as-error smoke tests. | Removed eager re-exports from `__init__.py` and added a fresh-process regression test with `RuntimeWarning` promoted to an error. |
